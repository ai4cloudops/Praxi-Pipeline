...
def print_metrics(cwd, y_true, y_pred, label):
    # with open(cwd+'index_label_mapping', 'rb') as fp:
...
# )
    ...
    train_tagset_files, train_feature_matrix, train_label_matrix = tagsets_to_matrix(train_tags_path, cwd=cwd)
    ...
    with open(cwd+'index_label_mapping', 'rb') as fp:
        labels = np.array(pickle.load(fp))
    print_metrics(cwd, test_label_matrix, pred_label_matrix, labels)

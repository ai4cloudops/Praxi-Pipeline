...
# generate_prediction_op.container.set_image_pull_policy("Always")

# https://github.com/kubeflow/pipelines/blob/a9ac0b95bcc69c6214ca4e5e3208c816096f0377/samples/contrib/ibm-samples/watson/watson_train_serve_pipeline.py#L26
...
def praxi_pipeline():
    vop = dsl.VolumeOp(
        name="snapshot-claim-6",
        resource_name="snapshot-claim-6",
        size="1Gi",
    ...
    prediction = generate_prediction_op(tagset.outputs["output_text"]).add_pvolumes({"/fake-snapshot": vop.volume}).apply(use_image_pull_policy())

    ...
    for test_sample_batch_idx in [0]:   # this is the initial idx of a test batch, i.e., if test batch size is 4*0.25 = 1, `test_sample_batch_idx`` means pick the 0-index element as the test batch.
        ...
        for dataset in ["data_4"]:
            ...
            for (with_filter, freq) in [(False, 100)]:
                for n_jobs in [32]:
                    for clf_njobs in [32]:
                        for n_models, sim_thr, test_batch_count in zip([25,25,25,25,25],[0.98, 0.9, 0.8, 0.7, 0.6],[1,1,1,1,1, 1]):
                                        ...
                                        for max_bin in [1]:
                                            for input_size, dim_compact_factor in zip([None],[1,1,1,1,1]):
                                                for shuffle_idx in range(1):
                                                    ...
                                                    all_tags_set_global = set()
                                                    all_tags_set_per_clf = list()
                                                    for index_tag_mapping in index_tag_mapping_path:
                                                        with open(index_tag_mapping, 'rb') as fp:
                                                            all_tags_set = set(pickle.load(fp))
                                                            all_tags_set_global.update(all_tags_set)
                                                            all_tags_set_per_clf.append(all_tags_set)
                                                    
                                                    shared_token_count = np.zeros(len(all_tags_set_global))
                                                    tag_index_mapping_global= {}
                                                    for tag_idx, tag in enumerate(list(all_tags_set_global)):
                                                        tag_index_mapping_global[tag] = tag_idx
                                                    for all_tags_set in all_tags_set_per_clf:
                                                        for tag in list(all_tags_set):

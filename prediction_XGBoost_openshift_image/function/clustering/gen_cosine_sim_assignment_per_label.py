...
def group_similar_entries_with_names_and_mean(similarity_matrix, names, threshold=0.9):
    ...
    for i in range(n):
        if i not in visited:
            ...
            visited.add(i)
            indices_group = {i}  # Track indices for calculating mean similarity
            ...
            # Calculate mean cosine similarity for the current group
            if len(indices_group) > 1:
                # Extract the submatrix of similarities for the current group
                submatrix = similarity_matrix[np.ix_(list(indices_group), list(indices_group))]
                np.fill_diagonal(submatrix, np.nan)  # Ignore self-similarities
                mean_similarity = np.nanmean(submatrix)
            else:
                mean_similarity = 1  # Default to 1 if the group has only one member

            groups.append(current_group)
            group_means.append(mean_similarity)

    return groups, group_means

def clustering(train_tags_init_path, cwd, n_samples, train_tags_init_l=None, threshold=0.9, train_packages_select_set=set(), tokens_filter_set=set(), input_size=None, compact_factor=1, freq=100):
    ...
    train_feature_matrix_init_per_label = []
    train_label_matrix_init_per_label = []

    # Iterate through the matrix in 'n' row chunks
    for start_row in range(0, train_feature_matrix_init.shape[0], n_samples):
        # Select 'n' rows
        end_row = min(start_row + n_samples, train_feature_matrix_init.shape[0])
        train_feature_matrix_init_row_slice = train_feature_matrix_init[start_row:end_row]
        train_label_matrix_init_row_slice = train_label_matrix_init[start_row:end_row]
        
        # Sum the selected rows and convert to dense format for display
        train_feature_matrix_init_row_slice_mean_rows = np.round(train_feature_matrix_init_row_slice.mean(axis=0))
        train_feature_matrix_init_per_label.append(scipy.sparse.csr_matrix(train_feature_matrix_init_row_slice_mean_rows))

        train_label_matrix_init_mean_rows = train_label_matrix_init_row_slice.mean(axis=0)
        train_label_matrix_init_per_label.append(train_label_matrix_init_mean_rows)

    # Convert list of arrays to a single numpy array for easier handling
    train_feature_matrix_init_per_label = scipy.sparse.vstack(train_feature_matrix_init_per_label)
    train_label_matrix_init_per_label = np.vstack(train_label_matrix_init_per_label)

    index_label_mapping_path = f"{cwd}index_label_mapping"
    labels = one_hot_to_names(index_label_mapping_path, train_label_matrix_init_per_label)
    labels = [labels[row_idx][0] for row_idx in range(len(labels))]

    # # Check if the number of rows is a multiple of n_samples
    # if train_feature_matrix_init.shape[0] % n_samples == 0:
    #     # Reshape the array into (-1, 10, arr.shape[1]) where -1 infers the necessary size
    #     # This groups every n_samples rows into one block
    #     train_feature_matrix_init_dense = train_feature_matrix_init.todense()
    #     reshaped_train_feature_matrix_init = train_feature_matrix_init_dense.reshape(-1, n_samples, train_feature_matrix_init_dense.shape[1])
    #     reshaped_train_label_matrix_init = train_label_matrix_init.reshape(-1, n_samples, train_label_matrix_init.shape[1])
        
    #     # Sum each block of n_samples rows
    #     train_feature_matrix_init_per_label = reshaped_train_feature_matrix_init.sum(axis=1)
    #     train_label_matrix_init_per_label = reshaped_train_label_matrix_init.sum(axis=1)

    #     index_label_mapping_path = f"{cwd}index_label_mapping"
    #     labels = one_hot_to_names(index_label_mapping_path, train_label_matrix_init_per_label)
    # else:
    #     print("The number of rows is not a multiple of 10. Please adjust the array accordingly.")
    #     sys.exit(-1)
    

    # Calculate pairwise cosine similarity
    similarity_matrix = cosine_similarity(train_feature_matrix_init_per_label)

    groups, group_means = group_similar_entries_with_names_and_mean(similarity_matrix, labels, threshold)
...
if __name__ == "__main__":
    ...
    packages_ll["data_4"]=[package.replace("==", "_v").replace(".","_") for package in packages_l]
    ...
    for test_sample_batch_idx in [0]:   # this is the initial idx of a test batch, i.e., if test batch size is 4*0.25 = 1, `test_sample_batch_idx`` means pick the 0-index element as the test batch.
        ...
        for dataset in ["data_4"]:
            ...
            # with open("/home/cc/Praxi-study/Praxi-Pipeline/data/"+dataset+"/filters/tagsets_SL_filtered_pair_tagsetfilenames_d_same_length", 'rb') as tf:
            ...
            for (with_filter, freq) in [(False, 100)]:
                if with_filter:
                    ...
                    # with open("/home/cc/Praxi-study/Praxi-Pipeline/data/"+dataset+"/filters/tagsets_SL_tokenshares_filter_set", 'rb') as tf:
                    #     tokenshares_filter_set_d[dataset] = yaml.load(tf, Loader=yaml.Loader)
                    # with open("/home/cc/Praxi-study/Praxi-Pipeline/data/"+dataset+"/filters/tagsets_SL_tokennoises_filter_set", 'rb') as tf:
                    ...
                    # Consider a token occurence dictionary to pick tokens by occurence in token space on the fly.
                    # with open(f"/home/cc/Praxi-study/Praxi-Pipeline/data/{dataset}/filters/tagsets_SL_tagnames_reoccurentcount_100lite_d", 'rb') as tf:
                ...
                for n_jobs in [32]:
                    for n_models, sim_thr, test_batch_count in zip([3000,3000, 3000, 3000],[0.99, 0.999, 0.9999],[1,1,1,1,1]):
                                ...
                                for tree_method in["exact"]: # "exact","approx","hist"
                                    for max_bin in [1]:
                                        for input_size, dim_compact_factor in zip([None],[1,1,1,1,1]): # [None, 6832, 13664, 27329, 54659,109319],[1,1,1,1] # [None, 500, 1000, 5000, 10000, 15000],[1,1,1,1,1,1]
                                            ...
                                            tags_path = "/home/cc/Praxi-study/Praxi-Pipeline/data/"+dataset+"/tagsets_SL/"
                                            train_subset = set(packages_l)
                                            labels_tagfiles_d, tag_files_l = defaultdict(list), []
                                            ...
                                            cwd = f"/home/cc/Praxi-study/Praxi-Pipeline/data/data_4_clustering/sim_thr{sim_thr}/"
                                            print(cwd)
                                            tagfiles_l = []
                                            for k,v in labels_tagfiles_d.items():
                                                tagfiles_l.extend(v)
                                            name_groups, groups_mean_similarity = clustering(tags_path, cwd, n_samples, train_tags_init_l=tagfiles_l, threshold=sim_thr)

                                            # Convert the list of sets to a list of lists for JSON serialization
                                            name_groups_list = [list(group) for group in name_groups]
                                            # Prepare the data for JSON
                                            data_to_save = {
                                                "name_groups": name_groups_list,
                                                "mean_similarities": groups_mean_similarity,
                                                # "labels": labels
                                            }
                                            # Save the data as JSON
                                            with open(f'{cwd}/name_groups.json', 'w') as json_file:

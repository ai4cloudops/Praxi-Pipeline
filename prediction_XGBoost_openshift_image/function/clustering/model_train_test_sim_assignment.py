    ...
    for test_sample_batch_idx in [0]:   # this is the initial idx of a test batch, i.e., if test batch size is 4*0.25 = 1, `test_sample_batch_idx`` means pick the 0-index element as the test batch.
        ...
        for dataset in ["data_4"]:
            ...
            for (with_filter, freq) in [(False, 100)]:
                ...
                for n_jobs in [32]:
                    for n_models, sim_thr, test_batch_count in zip([25, 25, 25, 25, 25, 25],[[0.98, 0.95, 0.9, 0.8, 0.7, 0.6, 0.55][1]],[1,1,1,1,1]):
                                    ...
                                    for max_bin in [1]:
                                        for input_size, dim_compact_factor in zip([None],[1,1,1,1,1]):
                                            with open(clustering_d[sim_thr], 'r') as openfile:
                                                ...
                                                n_models = min(len(name_groups["name_groups"]), n_models)
                                            ...
                                            for shuffle_idx in range(10):
                                                ...
                                                # target_group_size = len(package_subset) // n_models
                                                # if len(package_subset) % n_models:  # Adjust if we can't divide groups evenly
                                                #     target_group_size += 1

                                                # regrouped = []
                                                # for i in range(0, len(package_subset), target_group_size):
                                                #     # Combine the next 'target_group_size' groups into one
                                                #     combined_group = set()
                                                #     for group in package_subset[i:i + target_group_size]:
                                                #         combined_group.update(group)
        ...
        #     for (with_filter, freq) in [(False, 100)]:
        #         for n_jobs in [32]:
        #             for n_models, sim_thr, test_batch_count in zip([25, 25, 25, 25, 25],[[0.98, 0.9, 0.8, 0.7, 0.6, 0.55][2]],[1,1,1,1,1]):# [0.9, 0.95, 0.85, 0.8, 0.7]
        #                 for n_estimators in [100]:
        #                     for depth in [1]:
        #                         for tree_method in["exact"]: # "exact","approx","hist"
        #                             for max_bin in [1]:
        ...
        #                                     for shuffle_idx in range(1):
                                                ...
                                                for clf_njobs in [32]:
                                                    ...
                                                    for i in range(n_models):
                                                        clf_pathname = "/home/cc/Praxi-study/Praxi-Pipeline/prediction_XGBoost_openshift_image/model_testing_scripts/cwd_ML_with_"+dataset+"_"+str(n_models)+"_"+str(i)+"_train_"+str(shuffle_idx)+"shuffleidx_"+str(test_sample_batch_idx)+"testsamplebatchidx_"+str(n_samples)+"nsamples_"+str(clf_njobs)+"njobs_"+str(n_estimators)+"trees_"+str(depth)+"depth_"+str(input_size)+"-"+str(dim_compact_factor)+"rawinput_sampling1_"+str(tree_method)+"treemethod_"+str(max_bin)+"maxbin_modize_par_"+str(with_filter)+f"{freq}removesharedornoisestags_sim{sim_thr}assignment/model_init.json"

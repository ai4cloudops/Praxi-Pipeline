...
def clustering(train_tags_init_path, cwd, train_tags_init_l=None, threshold=0.9, train_packages_select_set=set(), tokens_filter_set=set(), input_size=None, compact_factor=1, freq=100):
    ...
    similarity_matrix = cosine_similarity(train_feature_matrix_init)

    groups, group_means = group_similar_entries_with_names_and_mean(similarity_matrix, train_tagset_files_init, threshold)
...
if __name__ == "__main__":
    ...
    for test_sample_batch_idx in [0]:   # this is the initial idx of a test batch, i.e., if test batch size is 4*0.25 = 1, `test_sample_batch_idx`` means pick the 0-index element as the test batch.
        ...
        for dataset in ["data_4"]:
            ...
            # # #############################################################
            for (with_filter, freq) in [(False, 100),(True, 100),(True, 300)]:
                if with_filter:
                ...
                for n_jobs in [32]:
                    for n_models, test_batch_count in zip([1000],[1,1,1,1,1]): #([50,25,20,15,10,5,1],[1,1,1,1,1,1,8]): # ([1,25,10],[8,1,1])
                                ...
                                for tree_method in["exact"]: # "exact","approx","hist"
                                    for max_bin in [1]:
                                        for input_size, dim_compact_factor in zip([None],[1,1,1,1,1]): # [None, 6832, 13664, 27329, 54659,109319],[1,1,1,1] # [None, 500, 1000, 5000, 10000, 15000],[1,1,1,1,1,1]
                                            ...
                                            cwd = "/home/cc/Praxi-study/Praxi-Pipeline/data/data_4_clustering/"

                                            train_name_groups, train_groups_mean_similarity = clustering(train_tags_path, cwd, train_tag_files_l, threshold=0.9)
                                            test_name_groups, test_groups_mean_similarity = clustering(test_tags_path, cwd, test_tag_files_l, threshold=0.9)

                                            # Convert the list of sets to a list of lists for JSON serialization
                                            train_name_groups_list = [list(group) for group in train_name_groups]
                                            test_name_groups_list = [list(group) for group in test_name_groups]
                                            # Prepare the data for JSON
                                            data_to_save = {
                                                "train_name_groups": train_name_groups_list,
                                                "train_mean_similarities": train_groups_mean_similarity,
                                                "test_name_groups": test_name_groups_list,
                                                "test_mean_similarities": train_groups_mean_similarity
                                            }

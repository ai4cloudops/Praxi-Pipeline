...
for dataset in ["data_4"]:
    ...
    for (with_filter, freq) in [[(False, 100),(True, 25)][1]]:
        for n_jobs in [32]:
            for clf_njobs in [32]:
                for n_models, sim_thr, test_batch_count in zip([1000, 500, 50, 25, 10, 1],[0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.7],[1,1,1,1,1,1,1]):
                                    ...
                                    for input_size, dim_compact_factor in zip([None],[1]): # [None, 10000, 1000, 500, 5000, 15000],[1,1,1,1,1,1] # [None, 13, 106, 284, 427, 854, 1138, 1708, 3416, 6832, 13664, 27329, 54659, 109319]
                                                    ...
                                                    if os.path.isfile(filename):
                                                        with open(filename) as file:
                                                            for line in file:
                                                                ...
                                                                if line_l[0] == "tagsets_to_matrix-trainset":
                                                                    data_d[str(n_models)+"_models_"+str(sim_thr)+"_sim_thr"][n_estimators][input_size]["encodetime"].append(float(line_l[1]))
                                                                if line_l[0] == "tagsets_to_matrix-trainset_xsize":
                                        ...
                                        # for shuffle_idx in range(3):
                                            for test_sample_batch_idx in [0]:
                                                ...
                                                # filename  ="/home/cc/Praxi-study/Praxi-Pipeline/prediction_XGBoost_openshift_image/model_testing_scripts_bak/False100_clustering/"+data_dir+"cwd_ML_with_"+dataset+"_"+str(n_models)+"_train_"+str(shuffle_idx)+"shuffleidx_"+str(test_sample_batch_idx)+"testsamplebatchidx_"+str(n_samples)+"nsamples_"+str(n_jobs)+"njobs_"+str(clf_njobs)+"clfnjobs_"+str(n_estimators)+"trees_"+str(depth)+"depth_"+str(input_size)+"-"+str(dim_compact_factor)+"rawinput_sampling1_"+str(tree_method)+"treemethod_"+str(max_bin)+"maxbin_modize_par_"+str(with_filter)+f"{freq}removesharedornoisestags_sim{sim_thr}assignment/metrics_pred.out"
                                                          # /home/cc/Praxi-study/Praxi-Pipeline/prediction_XGBoost_openshift_image/model_testing_scripts_bak/            cwd_ML_with_data_4     _50               _train_  0                 shuffleidx_  0                           testsamplebatchidx_  4               nsamples_32             njobs_  32              clfnjobs_  100                trees_  1           depth_  None             -  1                        rawinput_sampling1_  exact             treemethod_  1             maxbin_modize_par_  False              100   removesharedornoisestags_sim0.85assignment

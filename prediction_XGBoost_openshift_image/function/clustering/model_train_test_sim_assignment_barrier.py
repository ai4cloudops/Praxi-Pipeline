...
if __name__ == "__main__":
    ...
    for test_sample_batch_idx in [0]:   # this is the initial idx of a test batch, i.e., if test batch size is 4*0.25 = 1, `test_sample_batch_idx`` means pick the 0-index element as the test batch.
        ...
        #         for n_jobs in [32]:
        #             for n_models, sim_thr, test_batch_count in zip([25, 25, 25, 25, 25],[[0.98, 0.9, 0.8, 0.7, 0.6, 0.55][5]],[1,1,1,1,1]):
        ...
        for dataset in ["data_4"]:
            ...
            for (with_filter, freq) in [(False, 100)]:
                for n_jobs in [32]:
                    for clf_njobs in [32]:
                        for n_models, sim_thr, test_batch_count in zip([25, 25, 25, 25, 25],[[0.98, 0.9, 0.8, 0.7, 0.6, 0.55][4]],[1,1,1,1,1]):# [0.9, 0.95, 0.85, 0.8, 0.7]
                            for n_estimators in [100]:
                                for depth in [1]:
                                    for tree_method in["exact"]: # "exact","approx","hist"
                                        for max_bin in [1]:

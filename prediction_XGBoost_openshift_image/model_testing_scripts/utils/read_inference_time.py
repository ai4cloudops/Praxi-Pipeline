                    ...
                    for depth in [1]:
                        for tree_method in["exact"]: # "exact","approx","hist"
                            for max_bin in [1]:
                                ...
                                for input_size, dim_compact_factor in zip([6832, 27329, 109319],[1,1,1]): # [None, 10000, 1000, 500, 5000, 15000],[1,1,1,1,1,1] # [None, 13, 106, 284, 427, 854, 1138, 1708, 3416, 6832, 13664, 27329, 54659, 109319]
                                    for i in range(n_models):
                                        data_d[str(n_models)+"_models"][input_size//dim_compact_factor] = defaultdict(list)
                                        for shuffle_idx in range(3):
                                            for test_sample_batch_idx in [4,1,2,0,3]:
                                                filename  ="/home/cc/Praxi-study/Praxi-Pipeline/prediction_XGBoost_openshift_image/model_testing_scripts/data0_results/cwd_ML_with_"+dataset+"_"+str(n_models)+"_"+str(i)+"_train_"+str(shuffle_idx)+"shuffleidx_"+str(test_sample_batch_idx)+"testsamplebatchidx_"+str(n_samples)+"nsamples_"+str(n_jobs)+"njobs_"+str(n_estimators)+"trees_"+str(depth)+"depth_"+str(input_size)+"-"+str(dim_compact_factor)+"rawinput_sampling1_"+str(tree_method)+"treemethod_"+str(max_bin)+"maxbin_modize_par_removesharedornoisestags/metrics_init.out"
                                                if os.path.isfile(filename):
                                                    with open(filename) as file:
                                                        for line in file:
                                                            line_l = line.rstrip().split(":")
                                                            if line_l[0] == "BOW_XGB_init.fit":
                                                                data_d[str(n_models)+"_models"][input_size//dim_compact_factor]["traintime"].append(float(line_l[1]))
                                                            if line_l[0] == "tagsets_to_matrix-trainset_xsize":
                                                                data_d[str(n_models)+"_models"][input_size//dim_compact_factor]["samplesize"].append(float(line_l[1]))
                                                            if line_l[0] == "tagsets_to_matrix-trainset_ysize":
                                                                data_d[str(n_models)+"_models"][input_size//dim_compact_factor]["dimensions"].append(float(line_l[1]))
                                    for shuffle_idx in range(3):
                                                ...
                                                try:
                                                    with open(filename, "r") as f:
                                                        for line in f:
                                                            line_l = line.rstrip().split()
                                                            if len(line_l) == 9:
                                                                data_d[str(n_models)+"_models"][input_size//dim_compact_factor]["f1-score"].append((float(line_l[3])))
                                                                # data_d[str(n_models)+"_models"][input_size//dim_compact_factor]["f1-score-config"].append(str(shuffle_idx)+"shuffleidx_"+str(test_sample_batch_idx)+"testsamplebatchidx")
